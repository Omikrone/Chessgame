...
void GameSession::apply_engine_move(crow::websocket::connection& ws) {
    ...
    PositionResponse game_state = PositionMapper::to_position_response(state, _game.get_fen(), _game.get_current_turn());
...
Color GameSession::get_player_color() const {
    return _player_color;
}

int GameSession::get_id() const {
    return _id;
}