...
EngineSession::EngineSession(std::string engine_addr, int engine_port, int game_id)
    : _engine_addr(engine_addr), _engine_port(engine_port), _game_id(game_id) {
    ...
    auto con = _cli.get_connection(
        "ws://" + _engine_addr + ":" + std::to_string(_engine_port) + "/engine/" + std::to_string(_game_id), ec);
    if (ec) {
...
void EngineSession::on_message(websocketpp::connection_hdl /*hdl*/,
